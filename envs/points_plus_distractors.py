...
class PointDistractorEnv:
    def __init__(self, sparse_reward=False, horizon=200, fixed_reset=True):
        ...
        model = load_model_from_path("envs/pointshard.xml")
        sim = MjSim(model)
        self.sim = sim
        viewer = MjViewer(sim)
    ...
    def reset_fixed_goal(self, goal_idx):
    ...
    def get_random_state(self):
        qpos = 0.5*np.random.randn(self.model.nq)
    ...
    def get_reward(self):
        ...
        cyl_obs_dist = self.get_body_com("object") - self.get_body_com("goal_" + str(self.goal_idx))
