...
from policies import OneShotImitationPol
...
def run():
    ...
    buffer.update_rewards = buffer.update_rewards_v5
    ...
    imitation_pol = OneShotImitationPol(s_mb=s_mb, a_mb=a_mb)
    ...
    buffer.update_rewards(encoder=None, batch_size=batch_size)
    buffer.make_rl_data()
...
def run_evaluation(outer_iter_idx, outer_iters, expert_traj_dir, env_name, env, traj_horizon, imitation_pol,
    ...
    for traj_idx in range(len(expert_obs)):
        ...
        baseline_imitation_eval = False
        if baseline_imitation_eval:
            obs = env.reset()
            for time in range(traj_horizon):
            ...
            this_test_set_rews_baseline_pol.append(-1000.0)
    ...
    np.savez("results/" + env_name + "_rnn_baseline.npz", **d)
